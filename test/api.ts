...
suite('Hyperion API', function () {
    ...
    suite('v2', function () {
        test('get_abi_snapshot', async function () {
            const response = await hyperion.v2.get_abi_snapshot("eosio.token", 2000, true);
            assert.equal(response.abi.version, "eosio::abi/1.1")
        })
    
        test('get_voters', async function () {
            const response = await hyperion.v2.get_voters('teamgreymass', true, 100, 200);
        ...
        test('get_links', async function () {
            const response = await hyperion.v2.get_links('teamgreymass');
        ...
        test('get_proposals', async function () {
            const response = await hyperion.v2.get_proposals({
                skip: 1,
                limit: 10
            });
        ...
        test('get_actions', async function () {
            const response = await hyperion.v2.get_actions("teamgreymass", {
                filter: "eosio.token:*",
                skip: 100,
                limit: 5
            });
        ...
        test('get_created_accounts', async function () {
            const response = await hyperion.v2.get_created_accounts("teamgreymass");
        ...
        test('get_creator', async function () {
            const response = await hyperion.v2.get_creator("teamgreymass");
        ...
        test('get_deltas', async function () {
            const response = await hyperion.v2.get_deltas("eosio.token", "teamgreymass", "accounts", "teamgreymass");
        ...
        test('get_table_state', async function () {
            const response = await hyperion.v2.get_table_state("eosio.token", "stat", 267000000);
        ...
        test('get_key_accounts', async function () {
            const response = await hyperion.v2.get_key_accounts("EOS8KmhygTrrvtW7zJd6HXWrNqA5WX9NzScZ37JyXRiwpiJN2g2rR");
        ...
        test('get_tokens', async function () {
            const response = await hyperion.v2.get_tokens("teamgreymass");
        ...
        test('get_transaction', async function () {
            const response = await hyperion.v2.get_transaction("a51a3cc53b2ff5d5b25ad44b1e3ef5f796ce3ca60101ea05b3be64e68b684ccb");
